...
set BZIP2DIR=%SOURCE_ROOT%\bzip2-1.0.6
...
echo "Additional      : BZIP2DIR=%BZIP2DIR%"
...
if NOT EXIST %BZIP2DIR% (
   echo "%BZIP2DIR% does not exist."
...
b2 -j%NUMBER_OF_PROCESSORS% toolset=%msvc% architecture=x86 address-model=64 -s BZIP2_SOURCE=%BZIP2DIR% ^
   threading=multi runtime-link=shared --build-type=minimal link=static --stagedir=stage/x86_64 stage
b2 -j%NUMBER_OF_PROCESSORS% toolset=%msvc% architecture=x86 address-model=64 -s BZIP2_SOURCE=%BZIP2DIR% ^
   threading=multi runtime-link=shared --build-type=minimal link=shared --stagedir=stage/x86_64 stage install

goto end

:simple
b2 -j%NUMBER_OF_PROCESSORS% address-model=64 -s BZIP2_SOURCE=%BZIP2DIR% link=static --stagedir=stage/x64-static stage install
b2 -j%NUMBER_OF_PROCESSORS% address-model=64 -s BZIP2_SOURCE=%BZIP2DIR% link=shared --stagedir=stage/x64-shared stage install
