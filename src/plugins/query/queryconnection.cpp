...
QueryPlotData *
QueryConnection::fetch( const std::wstring& dataGuid )
{
#if 0
    ...
    if ( ( file = select_file( dataGuid, L"/Processed/Spectra" ) )
        ...
        auto d = std::make_shared< QueryPlotData >();
        ...
        for ( auto& att : atts ) {
            ...
            } else if ( att.dataClass() == adcontrols::QuerySample::dataClass() ) {

                d->sample = std::make_shared< adcontrols::QuerySample >();
                att.fetch( *d->sample );                
...
const adcontrols::QuerySequence *
QueryConnection::querySequence()
{
    if ( !sequence_ )
        readMethods();
...
bool
QueryConnection::readMethods()
{
#if 0
    sequence_ = std::make_shared< adcontrols::QuerySequence >();
    procmethod_ = std::make_shared< adcontrols::ProcessMethod >();

    if ( auto folder = fs_->findFolder( L"/Processed/Query" ) ) {
        ...
        do {
            auto it = std::find_if( files.begin(), files.end(), [] ( const adfs::file& t ){ return t.dataClass() == adcontrols::QuerySequence::dataClass(); } );
            if ( it != files.end() )
                it->fetch( *sequence_ );
        } while ( false );
    }
