...
QuanQuery::buildCountingQuery( std::string& query, int idx, bool isISTD, const std::string& additional )
{
    ...
    if ( isISTD ) {
        ...
        if ( hasIsCounting ) {
            fmtstr =
                ...
                ", intensity"
                ", timeCounts * 100.0 / trigCounts as 'CountRate', trigCounts, QuanResponse.amount, dataSource"
                " FROM QuanSample, QuanResponse, QuanCompound"
                " WHERE QuanSample.id = idSample"
                " AND QuanResponse.idCmpd = QuanCompound.uuid %1%) t1"
                " LEFT JOIN"
                " (SELECT idSample, timeCounts * 100.0 / trigCounts as 'CountRate',QuanResponse.fcn,QuanResponse.formula,QuanResponse.mass"
        ...
        fmtstr =
            ...
            ", sampleType, QuanCompound.formula, QuanResponse.fcn, QuanCompound.mass AS \"exact mass\""
            ", QuanResponse.mass"
            ", (QuanCompound.mass - QuanResponse.mass) * 1000 AS 'error(Da)'"
            ", timeCounts * 100.0 / trigCounts as 'CountRate', QuanResponse.amount, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound"
    ...
    if ( idx == 0 ) { // All
        ...
        fmtstr =
            ...
            ", timeCounts * 100.0 / trigCounts as 'count rate', QuanSample.level, QuanAmount.amount"
        ...
        fmtstr =
            ...
            ", QuanCompound.mass AS \"exact mass\", QuanResponse.mass"
            ", (QuanCompound.mass - QuanResponse.mass)*1000 AS 'error(mDa)'"
            ", timeCounts * 100.0 / trigCounts as 'counts/min', QuanSample.level, QuanAmount.amount"
            ", sampleType, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound, QuanAmount"
            " WHERE QuanSample.id = QuanResponse.idSample"
            " AND QuanResponse.idCmpd = QuanCompound.uuid"
...
QuanQuery::buildQuantifyQuery( std::string& query, int idx, bool isISTD, const std::string& additional )
{
    ...
    if ( idx == 0 ) { // All
        ...
        fmtstr = "SELECT QuanCompound.uuid, QuanResponse.id, QuanSample.name"
            ...
            ", QuanCompound.mass AS \"exact mass\", QuanResponse.mass"
            ", (QuanCompound.mass - QuanResponse.mass) * 1000 AS 'error(mDa)'"
            ", intensity, QuanResponse.amount, pkheight, pkwidth, ntp, asymmetry, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound"
            " WHERE QuanSample.id = QuanResponse.idSample"
            " AND QuanResponse.idCmpd = QuanCompound.uuid"
            " %1%"
        ...
        fmtstr = "SELECT QuanCompound.uuid, QuanResponse.id, QuanSample.name"
            ...
            ", QuanResponse.mass"
            ", (QuanCompound.mass - QuanResponse.mass) * 1000 AS 'error(mDa)'"
            ", intensity, QuanSample.level, QuanAmount.amount, pkheight, pkwidth, ntp, asymmetry, sampleType, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound, QuanAmount"
            " WHERE QuanSample.id = QuanResponse.idSample"
            " AND QuanResponse.idCmpd = QuanCompound.uuid"
            " AND sampleType = 1 "
