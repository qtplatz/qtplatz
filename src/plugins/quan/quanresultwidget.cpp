...
QuanResultWidget::CountingIndexChanged( int idx )
{
    ...
    if ( isISTD ) {
        query =
            ...
            ", intensity * 60000 / trigCounts as 'CountRate', trigCounts, QuanResponse.amount, QuanCompound.description, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound"
            " WHERE QuanSample.id = idSample"
            " AND QuanResponse.idCmpd = QuanCompound.uuid %1%) t1"
            " LEFT JOIN"
            " (SELECT idSample, intensity * 60000 / trigCounts as 'CountRate',QuanResponse.formula,QuanResponse.mass"
        ...
        query =
            ...
            ", intensity * 60000 / trigCounts as 'CountRate', QuanResponse.amount, QuanCompound.description, dataSource"
            " FROM QuanSample, QuanResponse, QuanCompound"
