...
using chemistry::SqlEditForm;

namespace {

    using chemistry::SqlEdit;
                                          ...
                                          , semiColonCaptured_( false )
{
    ...
    setSQL( "SELECT t1.id,svg,synonym,formula,mass,smiles,InChI,InChiKey FROM mols t1 LEFT OUTER JOIN synonyms t2 on t1.id = t2.id" );
}
