...
bool HelpPlugin::initialize(const QStringList &arguments, QString *error)
{
    ...
    QDir directory(fi.absolutePath()+"/qtPlatz");
    if (!directory.exists())
...
void HelpPlugin::extensionsInitialized()
{
    ...
    const QString filterInternal = QString("Qt Creator %1.%2.%3")
        .arg(IDE_VERSION_MAJOR).arg(IDE_VERSION_MINOR).arg(IDE_VERSION_RELEASE);
    const QRegExp filterRegExp("Qt Platz \\d*\\.\\d*\\.\\d*");
    ...
    //remove any qtplatz documentation that doesn't belong to current version
...
#else
                + QLatin1String("../../share/doc/qtPlatz/qtcreator.qch"));
#endif
