                        ...
                        if ( data.canConvert< portfolio::Folium >() ) {
                            ...
                            if ( portfolio::is_type< adutils::ChromatogramPtr >( folium ) ) {
                                if ( auto chro = portfolio::get< std::shared_ptr< adcontrols::Chromatogram > >( folium ) ) {
                                    if ( auto prop = chro->generatorProperty( "generator.extract_by_peak_info.pkinfo" ) ) {
                                        ADDEBUG() << *prop << "\n-------------------";
                                        if ( auto pk = adcontrols::MSPeakInfoItem::fromJson( *prop ) ) {
                                            ADDEBUG() << pk->toJson();
                                            ADDEBUG() << "----- " << pk->mass() << ", " << pk->area();
                                        }
                                        // boost::system::error_code ec;
                                        // auto jv = boost::json::parse( *prop, ec );
                                        // if ( !ec ) {
                                        //     ADDEBUG() << *prop;
                                        // }
                                    }
