...
void
ActionManager::handleContextChanged( const QList<Core::IContext*> & _t1, const Core::Context & _t2 )
{
    for ( auto& context : _t1 ) {
        ...
        if ( Core::IEditor * editor = qobject_cast<Core::IEditor *>(context) ) {
            boost::filesystem::path path = editor->document()->filePath().toStdWString();
