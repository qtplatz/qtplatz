...
Task::Task( size_t n ) : n_threads_(n)
                             , barrier_( n )
{
}
...
bool
Task::setConfiguration( const wchar_t * xml )
{
	if ( ! configXML_.empty() )
		return false;
	configXML_ = xml;
...
bool
Task::connect( Receiver_ptr receiver )
{
...
bool
Task::disconnect( Receiver_ptr receiver )
{
	internal::receiver_data data;
    data.receiver_ = Receiver::_duplicate( receiver );
...
////////////////////////////////////////////////////////////
bool
Task::connect( SignalObserver::ObserverEvents_ptr cb
...
bool
Task::disconnect( SignalObserver::ObserverEvents_ptr cb )
{
	acewrapper::scoped_mutex_t<> lock( mutex_ );
...
bool
Task::internal_initialize_mcast()
{
	if ( ! reactor() )
		internal_initialize_reactor();
	if ( ! mcast_handler_ ) {
...
void
Task::dispatch_debug( const std::wstring& text, const std::wstring& key )
{
...
void
Task::dispatch_debug( ACE_Message_Block * mblk )
{
...
void
Task::dispatch_mcast( ACE_Message_Block * mb )
{
...
void
Task::dispatch_sendto_device( const ACE_Message_Block * mb )
{
    ...
    default:
        assert(0);
        throw std::bad_cast( "bad data type for Task::dispatch_sendto_device" );
        break;
    }
}

void
Task::dispatch_query_device( const ACE_Message_Block * mb )
{
...
void
Task::device_update_notification( unsigned long clsId )
{
	acewrapper::scoped_mutex_t<> lock( mutex_ );
...
void
Task::adConfiguration( const TOFInstrument::ADConfigurations& v )
{
	acewrapper::scoped_mutex_t<> lock( mutex_ );
...
void
Task::setAnalyzerDeviceData( const TOFInstrument::AnalyzerDeviceData& d )
{
	acewrapper::scoped_mutex_t<> lock( mutex_ );
...
bool
Task::getAnalyzerDeviceData( TOFInstrument::AnalyzerDeviceData& d ) const
{
	acewrapper::scoped_mutex_t<> lock( const_cast< Task *>(this)->mutex() );
...
void
Task::push_profile_data( ACE_Message_Block * mb )
{
    if ( pObserver_ )
