...
namespace adwidgets {
    ...
    struct TimedEvent_AnyData : public std::enable_shared_from_this< TimedEvent_AnyData > {
        TimedEvent_AnyData( const EventCap* cap
                            , const QModelIndex& index
                            , const adcontrols::ControlMethod::any_type& value )
            : cap_( cap ), index_( index ), value_( value ) {
        }

        TimedEvent_AnyData( const QModelIndex& index
                            , const adcontrols::ControlMethod::any_type& value )
            : cap_( 0 ), index_( index ), value_( value ) {
        }
    ...
    struct TimedEventsWidget_painter : boost::static_visitor< void > {
        ...
        TimedEventsWidget_painter( QPainter * painter
                                   , const QStyleOptionViewItem& option
                                   , const QModelIndex& index )
            : painter_( painter ), option_( option ), index_( index ) {
        }
    ...
    template<> QVariant TimedEventsWidget_displayValue::operator()( const adcontrols::ControlMethod::delay_width_type& t ) const {
        return t.value.first * 1.0e6; // delay (s) -> us
    }
    template<> QVariant TimedEventsWidget_displayValue::operator()( const adcontrols::ControlMethod::any_type& t ) const {
        return QString( "" );
    }
    ...
    struct TimedEventsWidget_setModelData : boost::static_visitor< void > {
        ...
        const QModelIndex& index_;
        TimedEventsWidget_setModelData( QWidget * editor
                                        , QAbstractItemModel * model
                                        , const QModelIndex& index )
            : editor_( editor ), model_( model ), index_( index ) {
        }
    ...
    template<> void
    TimedEventsWidget_setModelData::operator()( const adcontrols::ControlMethod::voltage_type& ) const
    {
        try {
    ...
    TimedEventsWidget_setModelData::operator()( const adcontrols::ControlMethod::switch_type& t ) const
    {
        ...
        if ( auto editor = qobject_cast<QComboBox *>( editor_ ) ) {
            bool flag = ( editor->currentIndex() == 0 ) ? false : true;
            boost::get< adcontrols::ControlMethod::switch_type >( value ) = flag;
            QVariant v;
            v.setValue<>( value );
            model_->setData( index_, v, Qt::UserRole + 1 );
            model_->setData( index_, flag, Qt::EditRole );
        }
    }
    ...
    TimedEventsWidget_setModelData::operator()( const adcontrols::ControlMethod::choice_type& t ) const
    {
        ...
        if ( value.empty() )
            value = t;
        if ( auto editor = qobject_cast<QComboBox *>( editor_ ) ) {
            boost::get< adcontrols::ControlMethod::choice_type >( value ) = editor->currentIndex();
            QVariant v;
            v.setValue<>( value );
            model_->setData( index_, v, Qt::UserRole + 1 );
    ...
    TimedEventsWidget_setModelData::operator()( const adcontrols::ControlMethod::delay_width_type& t ) const
    {
        ...
        if ( index_.column() == TimedEventsWidget::impl::c_value )
            boost::get< adcontrols::ControlMethod::delay_width_type >( value ).value.first = time;
        else if ( index_.column() ==  TimedEventsWidget::impl::c_value_2 )
            boost::get< adcontrols::ControlMethod::delay_width_type >( value ).value.second = time;
        QVariant v;
        v.setValue<>( value );
        const auto& xvalue = boost::get< adcontrols::ControlMethod::delay_width_type >( value );
    ...
    public:
        ...
        QWidget * createEditor( QWidget * parent, const QStyleOptionViewItem &option, const QModelIndex& index ) const override {
            ...
            // AnyData will handle by editorEvent + Modeless dialog
                ...
                if ( value.type() == typeid( adcontrols::ControlMethod::any_type ) ) {
                    ...
                    if ( rc.contains( pt ) ) {
                        if ( auto cap = impl_.findEventCap( index ) ) {
                            ...
                            cap->edit_any( impl_.any_data_->value_, [this] ( const adcontrols::ControlMethod::any_type& a ) {
                                    if ( impl_.any_data_ )
                                        impl_.any_data_->callback( a );
                                } );
                            return true;
                        }
                    }
...
void
TimedEventsWidget::impl::refreshTable()
{
    adcontrols::ControlMethod::TimedEvents m;
    this_->getContents( m );
    std::sort( m.begin(), m.end(), []( const auto& a, const auto& b ){ return a.time() < b.time(); } );
...
TimedEventsWidget::impl::addLine( const adcontrols::ControlMethod::ModuleCap& modcap )
{
    ...
    double time = 0;
    if ( row )
        time = model_->index( row - 1, c_time ).data().toDouble();

    auto cap = modcap.eventCaps()[ 0 ];
    auto te = adcontrols::ControlMethod::TimedEvent( modcap, cap, time, cap.default_value() );
    model_->insertRow( model_->rowCount(), make_row( te ) );
}


QList< QStandardItem * >
TimedEventsWidget::impl::make_row( const adcontrols::ControlMethod::TimedEvent& e ) const
{
    ...
    // time (seconds)
    if ( auto item = new QStandardItem() ) {
        item->setData( e.time(), Qt::EditRole );
        items.push_back( item );
    }

    // module display_name, not editable
    if ( auto item = new QStandardItem() ) {
        item->setData( QString::fromStdString( e.modelDisplayName() ), Qt::EditRole );
        item->setEditable( false );
        items.push_back( item );
    }
    ...
    // item display_name
    if ( auto item = new QStandardItem() ) {
        item->setData( QString::fromStdString( e.itemDisplayName() ), Qt::EditRole );
        int capIdx = 0;
        item->setData( int( capIdx ), Qt::UserRole + 1 );
        items.push_back( item );
    }
    ...
    if ( auto item = new QStandardItem() ) {
        auto value = e.value();
        item->setData( boost::apply_visitor( TimedEventsWidget_displayValue(), value ), Qt::DisplayRole );
        QVariant v;
        v.setValue<>( value );
        item->setData( v, Qt::UserRole + 1 );
