...
ap240MethodTable::onInitialUpdate()
{
    ...
    model.setData( model.index( row, 1 ), m.bandwidth[0] );
...
ap240MethodTable::setContents( const ap240::method& m )
{
    ...
    model.setData( model.index( r_ch1_bandwidth, 1 ), m.bandwidth[0] );
...
ap240MethodTable::getContents( ap240::method& m )
{
    ...
    m.bandwidth[0] = model.index( r_ch1_bandwidth, 1 ).data().toInt();
    m.bandwidth[1] = model.index( r_ch1_bandwidth, 1 ).data().toInt();                
