...
namespace aqdrv4controller {
    ...
    public:
        ...
        uint32_t push_back( std::shared_ptr< acqrscontrols::aqdrv4::waveform > );
        void prepare_for_run( uint32_t, std::shared_ptr< const acqrscontrols::ap240::method > );

    private:
        std::vector< std::shared_ptr< so::DataReadBuffer > > que_;
        const boost::uuids::uuid objid_;
        std::deque< std::pair< uint32_t, std::shared_ptr< const acqrscontrols::ap240::method > > > methods_;
    };
}
