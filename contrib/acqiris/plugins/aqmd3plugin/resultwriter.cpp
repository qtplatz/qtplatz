...
#include <aqmd3controls/counting_data_writer.hpp>
#include <aqmd3controls/pkd_result.hpp>
#include <aqmd3controls/pkd_result_accessor.hpp>
...
{
    auto accessor = std::make_shared< aqmd3controls::pkd_result_accessor >();
#if 0
    do {
        std::lock_guard< std::mutex > lock( mutex_ );
        auto& cache = cache0_;
