...
namespace aqmd3widgets {
    ...
    class aqmd3Table::MyDelegate : public QStyledItemDelegate {

        aqmd3Table * pThis_;

    public:
        MyDelegate( aqmd3Table * p ) : pThis_( p ) {}
        ...
        void setModelData( QWidget * editor, QAbstractItemModel * model, const QModelIndex& index ) const override {
            if ( index.row() == r_sampling_rate ) {
                ...
                emit pThis_->valueChanged( idAQMD3NbrSamples, 0, index.data( Qt::EditRole ).toInt() );
...
aqmd3Table::~aqmd3Table()
{
    delete model_;
}

aqmd3Table::aqmd3Table(QWidget *parent) : adwidgets::TableView(parent)
...
aqmd3Table::onInitialUpdate()
{
    ...
    aqmd3controls::device_method m; // constructor default method for initial display
    int row = 0;
...
bool
aqmd3Table::setContents( const aqmd3controls::device_method& m )
{
    QStandardItemModel& model = *model_;
...
bool
aqmd3Table::getContents( aqmd3controls::device_method& m )
{
    QStandardItemModel& model = *model_;
    ...
    // digitizer_<...> will be override -- due to aqmd3 require self calibration for following two parameter change.
...
void
aqmd3Table::onHandleValue( idCategory id, int channel, const QVariant& value )
{
    QSignalBlocker block( this );
...
void
aqmd3Table::setEnabled( const QString& name, bool enable )
{
    if ( name == "StartDelay" ) {
