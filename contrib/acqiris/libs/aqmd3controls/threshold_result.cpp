...
threshold_result::write3( std::ostream& os, const threshold_result& t )
{
    ...
    if ( auto data = t.data() ) {
        ...
        os << boost::format( "\n%d, %d, %.8lf, %.8lf, 0x%08x, %.8lf, %d" )
            % data->serialnumber()
            % data->method().protocolIndex()
            % data->xmeta().initialXTimeSeconds
            % t.data()->epoch_time()
            % t.data()->well_known_events()
    ...
    namespace u5303a {
        ...
        std::ostream& operator << ( std::ostream& os, const threshold_result& t ) {
            ...
            if ( auto data = t.data() ) {
                ...
                os << boost::format( "\n%d, %.8lf, %.8lf, " )
                    % data->serialnumber() % data->xmeta().initialXTimeSeconds % t.data()->elapsed_time()
                   << t.data()->epoch_time()
                   << boost::format( ", 0x%08x" ) % t.data()->well_known_events()
                   << boost::format( ", %.8e, %.8e" ) % data->xmeta().scaleFactor % data->xmeta().scaleOffset
                   << boost::format( ", %.8e" ) % data->xmeta().initialXOffset;
