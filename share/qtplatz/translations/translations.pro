...
isEqual(QMAKE_DIR_SEP, /) {
    commit-ts.commands = \
        cd $$wd; \
        git add -N share/qtplatz/translations/*_??.ts && \
        for f in `git diff-files --name-only share/qtplatz/translations/*_??.ts`; do \
            $$LCONVERT -locations none -i \$\$f -o \$\$f; \
        done; \
        git add share/qtplatz/translations/*_??.ts && git commit
} else {
    commit-ts.commands = \
        cd $$wd && \
        git add -N share/qtplatz/translations/*_??.ts && \
        for /f usebackq %%f in (`git diff-files --name-only share/qtplatz/translations/*_??.ts`) do \
            $$LCONVERT -locations none -i %%f -o %%f $$escape_expand(\\n\\t) \
        cd $$wd && git add share/qtplatz/translations/*_??.ts && git commit
}
